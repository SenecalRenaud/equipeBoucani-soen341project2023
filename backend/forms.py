        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if (isRequesterAdmin := 'admin' in decoded_token and decoded_token['admin']) \
                        and permissions.get('admin',
                                            True) is False:  # Even admins are not able to proceed. Specific task, admin not required to access.
                    raise auth.InsufficientPermissionError(
                        "Admin does not need to and cant access this route; Permission admin=False is not fullfilled. "
                ...
                if 'myself' in permissions:
                    if (session_user := session.get("user", None)) is None or session_user['idToken'] != id_token:
                        raise auth.ExpiredSessionCookieError(
                            "Backend session could not be found and myself permission thus couldnt be verified; "
                            "Authorization denied.",
                            session_user
                        )
                    affected_user_uid = ""
                    ...
                    else:  # Similar handling can done for interviews, etc... future views using uids
                    ...
                    if bool(permissions['myself']) and decoded_token['sub'] != affected_user_uid:
                        raise auth.InsufficientPermissionError(
                            "Could not fullfill 'myself'=True perm. Action requested on user has to be done by that user. Authorization denied." + \
                        ...
                        raise auth.InsufficientPermissionError(
                            f"Could not fullfill 'myself'=False requirement. Action requested on user cant be done by "
                            f"that user. Authorization denied." + \
                            f"{decoded_token['sub']} was refused {request.path}",
                            session_user,
                            403
                        )
