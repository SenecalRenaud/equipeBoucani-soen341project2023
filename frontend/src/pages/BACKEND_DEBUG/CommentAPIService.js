...
export default class CommentAPIService {
    ...
    return await fetch(`http://localhost:5000/firebase-api/signin`, {
      ...
      .then((auth_json) => {
        ...
        if (!UserRESTAPI.checkIfObjectIsValidUser(auth_json)) {
          console.log(auth_json);
          throw new Error(
            "Auth_json not a valid user object... bad json promise"
          );
        }
      ...
      .then((auth_json) => {
        reducerDispatch({
          type: "REDUXACTION_LOGIN_SUCCESS",
          payload: auth_json,
        });
      ...
      .catch((error) => {
        reducerDispatch({ type: "REDUXACTION_LOGIN_ERROR", error });
  ...
  static async UserSignIn(reducerDispatch, formData) {
    try {
      const response = await fetch(
        `http://localhost:5000/firebase-api/signin`,
        {
          method: "POST",
          body: formData,
          credentials: "include", // To get cookies in AJAX request from backend origin
      ...
      await reducerDispatch({ type: "REDUXACTION_LOGIN_ERROR", error });
  ...
  static async GetUserDetails(uid) {
    return await fetch(`/firebase-api/get-user/${uid.toString()}/`, {
      method: "GET",
      //                  headers : {
      // //         'Content-Type':'application/json',
      // //     }
      ...
      .then((response) => {
        if (response.ok) {
          ...
          console.log(
            "no response after AJAX get user details CORS request to backend..."
          );
        }
      })
      .then((data) => (data === undefined ? {} : data.json()))
      .catch((err) =>
        console.log(
          "SOME ERROR OCCURED WHILE FETCHING USER ",
          uid,
          "'s DATA: ",
          err
        )
  ...
  static async UserLogout(
    reducerDispatch,
    { frontend_logout_only = false } = {}
  ) {
    if (!frontend_logout_only) {
      await fetch(`/firebase-api/logout`, {
        // if fails... try: http://localhost:5000?goto=logout
      })
        .then((response) => {
