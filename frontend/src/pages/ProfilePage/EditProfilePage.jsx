...
const EditProfilePage = () => {
  ...
  useEffect(() => {
    setFirstNameChanged(
      originalProfileData.firstName !== userProfileData.firstName
    );
    setLastNameChanged(
      originalProfileData.lastName !== userProfileData.lastName
    );
    setEmailChanged(originalProfileData.email !== userProfileData.email);
  }, [originalProfileData, userProfileData]);
  ...
  const handleSubmit = async (event) => {
    ...
    if (lastNameChanged) formData.append("lastName", userProfileData.lastName);
    if (emailChanged) formData.append("email", userProfileData.email);
    if (pfpChanged) {
    ...
    console.log(uid);
    await window.caches.open("profileEditing").then((cache) => {
      cache.put(
    ...
    return await fetch(`/firebase-api/edit-user/${uid}/`, {
      ...
      .then((response) => {
        ...
        const curr_url_path = location.pathname + location.search;
    ...
    >
      <container className="profile_container_1">
        ...
        {userProfileData.photo_url && (
          <img
            ...
            src={checkIfItemExists(userProfileData.photo_url)}
            alt="pfp"
        ...
        <label className="profile-content-username">
          <input
            className={firstNameChanged ? "editChanged" : ""}
            type="text"
            value={checkIfItemExists(userProfileData.firstName)}
            onChange={(event) => {
        ...
        <label className="profile-content-username">
          <input
            className={lastNameChanged ? "editChanged" : ""}
            type="text"
            value={checkIfItemExists(userProfileData.lastName)}
            onChange={(event) => {
              handleTextInputChange(event);
              setLastNameChanged(
                originalProfileData.lastName === userProfileData.lastName
        ...
        <h1 className="profile-usertype editDisabled">
          {" "}
        ...
        <h1 className="profile-email" style={{ float: `left` }}>
          {" "}
          <FontAwesomeIcon icon={faEnvelope} />
          &nbsp;
          <input
            className={emailChanged ? "editChanged" : ""}
            type="email"
            value={checkIfItemExists(userProfileData.email)}
            style={{ width: "85%" }}
            onChange={(event) => {
              handleTextInputChange(event);
              setEmailChanged(
                originalProfileData.email === userProfileData.email
              );
            }}
            name="email"
          />
        ...
        <h1 className="profile-content-lastSeen editDisabled">
          {new Date(parseInt(userProfileData.lastSeenEpoch)).toLocaleString(
            "us-en",
            DATETIME_OPTIONS
          )}
        </h1>
        ...
        <h1 className="profile-content-dateAdded editDisabled">
          {checkIfItemExists(
            new Date(parseInt(userProfileData.creationEpoch)).toLocaleString(
              "us-en",
              DATETIME_OPTIONS
            )
          )}
      ...
      <container className="profile_container_2">
        ...
        <textarea className="profile-content-introduction" cols="80" rows="20">
          My name is [insert name] and I am a young aspiring software engineer.
          From a very young age, I have been fascinated by the world of
          technology and computers, and I have always been intrigued by how
          software works. As I grew up, I began exploring different programming
          languages, experimenting with various coding projects, and expanding
          my knowledge of computer science. My passion for software engineering
          grew stronger as I started to see how it can positively impact
          people's lives. I believe that software engineering has the potential
          to solve some of the world's most pressing problems and make a
          significant difference in society. As a result, I am determined to
          become a skilled software engineer and contribute to the field in a
          meaningful way. I am excited about the endless possibilities that
          software engineering offers and I look forward to learning more and
          expanding my skills. I am committed to working hard and pursuing my
          dreams, and I am confident that with dedication and effort, I can make
          a meaningful contribution to the world of software engineering. Thank
          you for reading and I hope to share more of my journey with you soon.
        </textarea>
      </container>
      ...
      <container className="profile_container_3">
        ...
        {userProfileData.resume_url && (
          <div>
            <a
              href={userProfileData.resume_url}
              target="_blank"
              rel="noopener noreferrer"
              style={{ color: "orangered", textDecoration: "underline" }}
            >
              View Resume in other tab
            </a>
            <iframe
              className="profile_resume"
              src={userProfileData.resume_url}
              alt="resume"
      ...
      <container className="profile_container_4">
        ...
        <span className="hoverMessage">
          {" "}
          Saving all changes and update profile{" "}
        </span>
        <button
          onClick={(e) => {
            history(`/profile/${uid}`);
        ...
        <span className="hoverMessage">
          {" "}
          Cancel all changes and return to profile{" "}
        </span>
      </container>
    </form>
  );
