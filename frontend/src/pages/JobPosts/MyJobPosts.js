...
function MyJobPosts(props) {
  ...
  const updateSearchInput = async (searchBarInput) => {
    ...
    for (const [fieldName, allFieldInstances] of Object.entries(defaultData)) {
      allFieldInstances.forEach((fieldVal, dataIx) => {
        if (
          fieldVal != null &&
          fieldVal
            .toString()
    ...
    const filtered = Array.from(Object.values(defaultData)).map((row) =>
      row.filter((fieldVal, dataIx) => filteredIndicesHashSet.has(dataIx))
    );

    setSearchBarInput(searchBarInput);
    setData(
      //TODO COULD MAKE MORE EFFICIENT DATA-STRUCTURE/FUNCTIONAL PROGRAMMING WISE... BUT THIS STILL WORKS FOR THE PROJECT
    ...
    document.getElementById(
      "searchResultCount"
    ).innerText = `Found ${filteredIndicesHashSet.size} results`;
  };

  useEffect(() => {
    fetch("/getjob?mapAsFields=true")
      .then((response) => response.json())
      .then((data) => {
        const otherEmployerIxToRemove = []; //Must be list, since splice needs to avoid indices shifting
        data.employerUid.forEach((employerUid, postingIx) => {
  ...
  if (er || typeof data.id === "undefined") {
    // Json request body not loaded properly if not job post ID
    if (
      errorString.startsWith("SyntaxError") // || errorString === "SyntaxError: Unexpected token 'P', \"Proxy error\"... is not valid JSON"
    ) {
      return (
        <div className="post-comment-container">
          ...
          <div className="job-posts">
            <p align="center" style={{ color: "#FF5733" }}>
              Your API/backend server is not launched. Please ask an admin to
              launch the server to use this page.
            </p>
          </div>
        </div>
      );
      ...
      return (
        <div className="post-comment-container">
          ...
          <hr />
          <h3 align="center" style={{ color: "#8B8000" }}>
            No posts in the job_post table.
          </h3>
        </div>
      );
    }
    ...
    return (
      <div className="post-comment-container">
        ...
        >
          ...
          <SearchBar keyword={searchBarInput} setKeyword={updateSearchInput} />
          &emsp;
          <FontAwesomeIcon
            ...
            icon={faFilter}
            onClick={(e) => {
              alert("Filters are not ready to be implemeted yet! Soon !");
            }}
          />
        </section>
        <span
        ...
        >
          <b id="searchResultCount">
            {" "}
            Found {data.id ? data.id.length : 0} results.
          </b>
        </span>
        ...
        <div className="job-posts">
          {data.id &&
            data.id.map((id, i) => {
              ...
              return url_params.uid === data.employerUid[i] ? (
                <JobPostCard
                  key={i}
                  ...
                  editDate={new Date(
                    Date.parse(data.editDate[i])
                  ).toLocaleString()}
                  employerUid={data.employerUid[i]}
                  startDateProp={new Date(randomDate).toLocaleString("us-en", {
                    month: "long",
                    day: "numeric",
                    year: "numeric",
                  })}
                  endDateProp={new Date(randomDateEnd).toLocaleString("us-en", {
                    month: "long",
                    day: "numeric",
                    year: "numeric",
                  })}
