  ...
  useEffect(() => {
    fetch(`/getjob/${jobPostId}/`)
      ...
      .then((data) => {
        ...
        return Promise.all([
          CommentAPIService.GetUserDetails(data.employerUid),
  ...
  return (
    <>
      <CardArticle>
          ...
          <ComponentDialog
            button_component={
              <CardTitle style={{ textDecoration: "underline" }}>
                View Employer's job post ID #{jobPostId}{" "}
              </CardTitle>
            }
            content_component={<JobPostCard {...jobData} />}
          />
        </div>
        ...
        <CardDate>
          Date Applied:{" "}
          {new Date(date).toLocaleString("us-en", DATETIME_OPTIONS)}
        </CardDate>
