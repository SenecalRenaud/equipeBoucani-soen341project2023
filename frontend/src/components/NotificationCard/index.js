...
const NotificationCard = ({
  ...
  useEffect(
    () => {
      CommentAPIService.GetUserDetails(applicantUid).then((data) => {
  ...
  const HandleAccept = async (event) => {
    await JobPostingAPIService.sendAcceptedNotification({
  ...
  const HandleReject = async (job_id) => {
    await JobPostingAPIService.sendRejectedNotification({
      email: applicantUser.email,
      job_title: jobData.title,
    }).catch((error) =>
      console.log("Following error occurred after fetching from API: ", error)
