...
export default function UserRelationPermsFSM(user1, user2) {
  ...
  const isSelf = +(user1.email === user2.email) << 4;
  const ub1 = USER_TYPE_BIT[user1.userType];
  const ub2 = USER_TYPE_BIT[user2.userType] << 2;
  ...
  switch (isSelf | ub2 | ub1) {
    ...
    case 0b10101: //Applicant views itself
      return (
        <>
          <li>
            {" "}
            <Link to={`/viewapplications/${user1.uid}`}>
              {" "}
              My Applications{" "}
            </Link>{" "}
          </li>
          <li>
            {" "}
            <Link to="/current/interviews"> My Interviews </Link>{" "}
          </li>
        </>
      );
    case 0b01001: //Applicant views Employer
      return (
        <>
          <li>
            {" "}
            <Link to={`/viewjobposts/${user2.uid}`}>
              {user2.firstName} {user2.lastName}'s Job Postings{" "}
            </Link>{" "}
          </li>
        </>
      );
    ...
    case 0b00110: //Employer views Applicant
      return (
        <>
          <li>
            <Link to={`/${user2.uid}/makeOffer`}>
              {" "}
              Make {user2.firstName} {user2.lastName} an offer
            </Link>
          </li>
        </>
      );
    ...
    case 0b11010: //Employer views itself
      return (
        <>
          <li>
            {" "}
            <Link to={`/viewjobposts/${user1.uid}`}> My Job Postings </Link>
          </li>
          <li>
            {" "}
            <Link to="/current/interviews"> My Interviews </Link>
    ...
    case 0b00111: //Admin views Applicant
      return (
        <>
          <li>
            {" "}
            <Link to={`/viewapplications/${user2.uid}`}>
              {" "}
              {user2.email}'s Applications{" "}
            </Link>{" "}
          </li>
          <li>
            {" "}
            <Link to={`/${user2.uid}/interviews`}>
              {" "}
              {user2.email}'s Interviews{" "}
            </Link>{" "}
          </li>
        </>
      );
    ...
    case 0b11111: //Admin views itself
      return (
        <>
          <li>
            {" "}
            <Link to={"/admin_commandpanel"}> Admin Command Panel </Link>
          </li>
        </>
      );
    default:
