...
const UserDropDownMenu = ({ triggerMenuMarkup, triggeredUserUid }) => {
  ...
  const handleClick = useCallback(
    //memoizes on load the hook based on state, better sync click
    () => {
  ...
  useEffect(
    () => {
      CommentAPIService.GetUserDetails(triggeredUserUid).then((data) => {
        // console.log(data)
        setOtherUser(data);
      });
    },
  ...
  if (!state.isAuthenticated) {
    //!UserRESTAPI.checkIfObjectIsValidUser(user)
    user = {};
  ...
  const permissionBasedMenuOptionsMarkup = useMemo(
    () => UserRelationPermsFSM(user, otherUser),
    [user, otherUser]
  );

  const isNotUndetermined = !!user.email && !!otherUser.email;
  ...
  return (
    <div className="menu-container">
      ...
      <nav
        ref={dropDownRef}
        className={`menu ${isToggled ? "active" : "inactive"}`}
      >
        <ul>
          {isNotUndetermined && (
            <>
              <li>
                <Link to={`/profile/${triggeredUserUid}`}> View Profile </Link>
              </li>
              {/*<li><a href="/preferences"> Settings & Preferences</a></li>*/}
              <li>
                <Link to={`/notifications/${triggeredUserUid}`}>
                  {" "}
                  Notifications{" "}
                </Link>
              </li>
          ...
          <LogOutTab
            condition={isNotUndetermined && user.email === otherUser.email}
          />
        </ul>
      </nav>
    </div>
