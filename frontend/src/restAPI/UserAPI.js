...
export default class UserRESTAPI {
  ...
  static parseCurrentUserObjFromFrontendCache() {
    const userObj = {};
    const keys = USER_FIELDS; //Object.keys(localStorage),
    let i = keys.length;
  ...
  static getAllUserCookies() {
    const cookiesObj = {};
    const keys = USER_COOKIES; //Object.keys(localStorage),
    let i = keys.length;
  ...
  static async parseUserFromUidBackendRequest(uid, callback) {
    return await CommentAPIService.GetUserDetails(uid).then((user_json) =>
      callback(user_json)
    );
  }
  static checkIfObjectIsValidUser(obj) {
    for (const property of USER_FIELDS) {
      if (
  ...
  static updateCachedFrontendUserFromBackendSession({ userObj = null }) {
    ...
    const _userLoggedInBackendSession =
      userObj == null ? UserRESTAPI.userLoggedInBackendSession : userObj;
  ...
  static checkIfAllUserFrontendCacheAvailable() {
    const keys = USER_FIELDS;
    let i = keys.length;

    while (i--) {
      if (
        localStorage.getItem(keys[i]) == null ||
  ...
  static areAllUserCookiesAvailable() {
    const keys = USER_COOKIES;
    let i = keys.length;

    while (i--) {
      if (Cookies.get(keys[i]) == null || Cookies.get(keys[i]) === undefined) {
        return false;
      }
    }
    return true;
