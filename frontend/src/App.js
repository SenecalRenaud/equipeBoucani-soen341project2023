...
function App() {
  ...
  const postedComment = (comment) => {
    const new_comments = [...comments, comment];
  ...
  useEffect(() => {
    (async function assertFrontendAuthIsSyncedWithBackendSession() {
      ...
      const [isInBackendSession, userLoggedInBackendSession] =
        await UserRESTAPI.isUserLoggedInBackendSession();
      // UserRESTAPI.userLoggedInBackendSession = userLoggedInBackendSession;
      if (isInBackendSession && state === defaultState) {
        dispatch({
          type: "REDUXACTION_LOGIN",
          payload: userLoggedInBackendSession,
        });
      } else if (!isInBackendSession && state !== defaultState) {
        dispatch({ type: "REDUXACTION_LOGOUT" });
      } else if (!isInBackendSession && state === defaultState) {
        if (UserRESTAPI.areAnyUserCookiesAvailable()) {
          // MEANS BACKEND IS LOGGED OFF, BUT FRONTEND NOT COMPLETELY

          await CommentAPIService.UserLogout(dispatch, {
            frontend_logout_only: true,
          }); //Clean up frontend auth only
      ...
      if (
        Object.keys(userLoggedInBackendSession).length === 0 &&
        !UserRESTAPI.checkIfAllUserFrontendCacheAvailable() &&
        !UserRESTAPI.areAllUserCookiesAvailable()
      ) {
        // No user logged in backend session
        console.log("CCCCCCCCCCCCC");
        CommentAPIService.UserLogout(dispatch, { frontend_logout_only: true }); //Clean up frontend auth only

        return;
      }
      if (!UserRESTAPI.areAllUserCookiesAvailable()) {
        // Some authentification / token cookies are missing, frontend unauthorized
        console.log("BBBBBBBB");
        console.log(Object.keys(userLoggedInBackendSession).length === 0);

        alert(
          "BOUCANI SECURITY: Some User Auth / Token Cookies manual alterations were detected. \n\r\tImmediate logout. \n\r\t Status: 401"
        );
      ...
      try {
        userLoggedInFrontendAuth =
          UserRESTAPI.parseCurrentUserObjFromFrontendCache();
        //console.log(UserRESTAPI.parseCurrentUserObjFromFrontendCache())
        if (Object.keys(userLoggedInFrontendAuth).length === 0)
          throw new Error(
            "Incomplete (or empty) frontend session cache for logged in user descriptor"
          );
      } catch (frontendCachedUserIncompleteError) {
        console.log(frontendCachedUserIncompleteError);
        console.log("AAAAAAAAAAAA");
        alert(
          "BOUCANI SECURITY: Some Local User Descriptor / Cache  manual alterations were detected. \n\r\tImmediate logout. \n\r\t Status: 401"
        );
  ...
  return (
    <Router>
      ...
      <Routes>
        ...
        <Route
          path="/jobposting"
          element={<JobPostingForm postedJob={postedJob} />}
        />

        <Route path="/apply" element={<Apply />} />
        <Route
          path="/BACKEND_DEBUG"
          element={<PostCommentForm postedComment={postedComment} />}
        />
        <Route
          path="/signup"
          element={
        ...
        <Route
          path="/viewjobposts"
          element={<ViewJobPosts postedJob={postedJob} />}
        />
        <Route
          exact
          path="/viewjobposts/:uid"
          element={<MyJobPosts postedJob={postedJob} />}
        />
